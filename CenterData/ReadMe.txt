========================================================================
    活动模板库 : CenterData 项目概述
========================================================================

应用程序向导已为您创建了此 CenterData 项目，作为编写可执行程序 (EXE) 
的起点。
...
CenterData.idl
    ...
    此文件将由 MIDL 编译器处理以生成：
        C++ 接口定义和 GUID 声明		(CenterData.h)
        GUID 定义			(CenterData_i.c)
        类型库				(CenterData.tlb)
        封送处理代码			(CenterData_p.c 和 
                                           dlldata.c）

CenterData.h
    此文件包含 CenterData.idl 中定义的项目的 C++ 接口定义
...
StdAfx.h，StdAfx.cpp
    这些文件用于生成名为 CenterData.pch 的预编译头 (PCH) 文件和
...
CenterDataps.vcxproj
    此文件是用于生成代理/存根 (stub) DLL 的项目文件（若有必要）。主项目中的 IDL 
    文件必须至少包含一个接口，并且在生成代理/存根 (stub) DLL 之前必须先编译 IDL 
    文件。此进程生成 dlldata.c、CenterData_i.c 和 
    CenterData_p.c，这些文件是生成代理/存根 (stub) DLL 所必需的。

CenterDataps.vcxproj.filters
