...
应用程序向导已为您创建了此 BaseHog 项目，作为编写动态链接库 (DLL) 
的起点。
...
BaseHog.idl
    ...
    此文件将由 MIDL 编译器处理以生成：
        C++ 接口定义和 GUID 声明		(BaseHog.h)
        GUID 定义			(BaseHog_i.c)
        类型库				(BaseHog.tlb)
        封送处理代码			(BaseHog_p.c 和 
                                           dlldata.c）

BaseHog.h
    此文件包含 BaseHog.idl 中定义的项目的 C++ 接口定义
...
StdAfx.h，StdAfx.cpp
    这些文件用于生成名为 BaseHog.pch 的预编译头 (PCH) 文件和
...
BaseHogps.vcxproj
    ...
    文件。此进程生成 dlldata.c、BaseHog_i.c 和 
    BaseHog_p.c，这些文件是生成代理/存根 (stub) DLL 所必需的。

BaseHogps.vcxproj.filters
