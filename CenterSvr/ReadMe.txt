========================================================================
    活动模板库 : CenterSvr 项目概述
========================================================================

应用程序向导已为您创建了此 CenterSvr 项目，作为编写可执行程序 (EXE) 
的起点。
...
CenterSvr.idl
    ...
    此文件将由 MIDL 编译器处理以生成：
        C++ 接口定义和 GUID 声明		(CenterSvr.h)
        GUID 定义			(CenterSvr_i.c)
        类型库				(CenterSvr.tlb)
        封送处理代码			(CenterSvr_p.c 和 
                                           dlldata.c）

CenterSvr.h
    此文件包含 CenterSvr.idl 中定义的项目的 C++ 接口定义
...
StdAfx.h，StdAfx.cpp
    这些文件用于生成名为 CenterSvr.pch 的预编译头 (PCH) 文件和
...
CenterSvrps.vcxproj
    ...
    文件。此进程生成 dlldata.c、CenterSvr_i.c 和 
    CenterSvr_p.c，这些文件是生成代理/存根 (stub) DLL 所必需的。

CenterSvrps.vcxproj.filters
